    ...
    <div className="relative inline-flex shrink-0 items-center">
      ...
      <div className={`relative shrink-0 ${isBanner ? "ml-1" : "ml-1 sm:ml-1"}`}>
        <select
          ...
          className={
            isBanner
              ? "min-w-[6.75rem] appearance-none bg-transparent border-0 outline-none ring-0 shadow-none text-on-banner font-semibold text-body-sm leading-normal py-0.5 pl-0 pr-6 cursor-pointer whitespace-nowrap hover:opacity-80 focus:outline-none focus:ring-0 disabled:opacity-60"
        ...
        <span
          className={`material-symbols-outlined pointer-events-none absolute right-0 top-1/2 -translate-y-1/2 icon-sm ${
            isBanner ? "text-on-banner/80" : "text-on-surface-variant"
          }`}
          aria-hidden
        >
          expand_more
