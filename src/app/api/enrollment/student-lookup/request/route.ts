...
export async function POST(request: Request) {
  const rateLimit = applyRequestRateLimit(`student-lookup-request:${getRequestIp(request)}`);
  ...
  const result = await requestPublicEnrollmentStudentVerification(String(body.email ?? ""));
